...
void Chart::OnPaint(wxPaintEvent& evt)
{
    ...
    if (gc && values.size() > 0)
    {
        ...
        if(evt.GetId() == 121)
            gc->SetPen(wxPen(wxSystemSettings::GetAppearance().IsDark() ? wxTheColourDatabase->Find("PINK") : *wxBLUE, 3));
        else
            gc->SetPen(wxPen(wxSystemSettings::GetAppearance().IsDark() ? wxTheColourDatabase->Find("LIGHT BLUE") : *wxBLUE, 3));
