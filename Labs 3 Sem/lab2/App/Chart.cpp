...
Chart::Chart(wxWindow* parent, wxWindowID id, const wxPoint& pos, const wxSize& size) : wxWindow(parent, id, pos, size, wxFULL_REPAINT_ON_RESIZE)
{
    this->SetBackgroundStyle(wxBG_STYLE_PAINT);

    this->Bind(wxEVT_PAINT, &Chart::OnPaint, this);
}

void Chart::OnPaint(wxPaintEvent& evt)
{
    ...
    if (gc && values.size() > 0)
    {
        ...
        gc->SetFont(titleFont, wxSystemSettings::GetAppearance().IsDark() ? *wxBLACK : *wxBLACK);
        ...
        gc->SetFont(*wxNORMAL_FONT, wxSystemSettings::GetAppearance().IsDark() ? *wxBLACK : *wxBLACK);
        ...
        int iddd = evt.GetId();
        if(evt.GetId() == 121)
            gc->SetPen(wxPen(wxSystemSettings::GetAppearance().IsDark() ? wxTheColourDatabase->Find("GREEN") : *wxGREEN, 3));
        else
            gc->SetPen(wxPen(wxSystemSettings::GetAppearance().IsDark() ? wxTheColourDatabase->Find("BLUE") : *wxBLUE, 3));
...
std::tuple<int, double, double> Chart::calculateChartSegmentCountAndRange(double origLow, double origHigh) {
    ...
    // return some defaults in case range Mults and maxSegments are mismatched
